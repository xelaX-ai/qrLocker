    ...
    <div className="min-h-screen bg-neutral-50">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
        <div className="grid grid-cols-3 gap-3 mb-8">
          ...
          {[{ label: "Всего", value: stats.total }, { label: "Свободно", value: stats.available }, { label: "Занято", value: stats.occupied }].map((s) => (
        ...
        {filtered.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            ...
            {filtered.map((locker) => (<LockerCard key={locker.id} locker={locker} />))}
          </div>
        )}
      </main>
      ...
      {showCreateModal && <CreateLockersModal onClose={() => setShowCreateModal(false)} onCreated={() => { setShowCreateModal(false); window.location.reload(); }} />}
    </div>
  );
}

function CreateLockersModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void; }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl border border-neutral-100 p-6 w-full max-w-sm mx-4 shadow-xl">
        <div className="flex items-center justify-between mb-5">
          ...
          <button onClick={onClose} className="text-neutral-400 hover:text-neutral-900 transition-colors cursor-pointer">
        ...
        {!createdLockers ? (
          ...
          <>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={onCreated} className="flex-1 py-2.5 text-sm font-medium text-neutral-600 bg-neutral-100 hover:bg-neutral-200 rounded-xl transition-all cursor-pointer">Готово</button>
            </div>
          </>
        )}
      </div>
