      ...
      <main className="max-w-lg mx-auto px-4 py-8 space-y-4">
          ...
          <div className="border-t border-neutral-100 pt-5">
            ...
            {isEditing ? (
              <div className="flex gap-2">
                <input type="text" value={ownerInput} onChange={(e) => setOwnerInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleAssign()} placeholder="Имя и фамилия..." autoFocus className="flex-1 px-3 py-2 text-sm border border-neutral-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-neutral-900 focus:border-transparent" />
                ...
                <button onClick={() => { setIsEditing(false); setOwnerInput(locker.owner_name ?? ""); }} className="px-3 py-2 text-sm text-neutral-500 hover:text-neutral-900 transition-colors cursor-pointer">Отмена</button>
              </div>
            ) : (
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => { setOwnerInput(locker.owner_name ?? ""); setIsEditing(true); }} className="text-xs text-neutral-400 hover:text-neutral-900 transition-colors cursor-pointer">Редактировать</button>
              </div>
            )}
          </div>
        ...
        <div className="bg-white rounded-2xl border border-neutral-100 p-5">
          ...
          <div className="space-y-2.5">
            <ActionButton onClick={() => { setOwnerInput(locker.owner_name ?? ""); setIsEditing(true); }} disabled={loading} icon={<path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />}>
